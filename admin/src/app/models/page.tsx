...
export default function ModelsPage() {
  ...
  const saveProvider = async () => {
    ...
    try {
      ...
      setForm({ id: "", name: "", base_url: "", auth_method: "bearer", api_key: "", api_key_header: "", protocol: "openai" });
      await refresh();
    } catch (e) {
  ...
  return (
    <>
      ...
      <p className="page-sub">注册任意 OpenAI 兼容 / Anthropic 源，按能力槽位接入，岗位/任务级路由</p>
        ...
        {providerFormOpen && (
          <div className="form-grid" style={{ marginBottom: 16 }}>
            <div className="form-row">
              <label>
                ID（slug）
                <input value={form.id} onChange={(e) => setForm({ ...form, id: e.target.value })} placeholder="myrelay" />
              </label>
              <label>
                显示名
            ...
            <button onClick={() => void saveProvider()}>保存 Provider</button>
          </div>
        )}
              ...
              {providers.map((p) => (
                <tr key={p.id}>
                  ...
                  <td>
                    ...
                    <span className="meta" style={{ marginLeft: 8 }}>{probe[p.id] || ""}</span>
                  </td>
                </tr>
              ))}
            </tbody>
